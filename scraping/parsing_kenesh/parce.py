...
def get_deps_links(html):
    ...
    for dep in deputaty:
        a = dep.find('a', class_='name').get('href')
...
def main():
    prepare_csv()
    links = get_all_links()
    # for link in links: # выполняется последовательно
    #     data = get_page_data(link)
    #     write_csv(data)
    start = datetime.now()
    with Pool(15) as pool: # выполняется параллельно
        pool.map(make_all, links)
    finish = datetime.now()
    print(f'Parsing занял: {finish - start}')

if __name__ == '__main__':
    main()
# get_page_data('http://kenesh.kg/ru/deputy/show/258/tulobaev-balbak-zarlikovich')
